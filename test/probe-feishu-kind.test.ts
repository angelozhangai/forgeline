// probeFeishu 必须把 adapter（port.probe）的 kind（auth/drift）透传到 ProbeResult。
// 回归点：之前 probeFeishu 丢了 kind → 飞书 token 过期/群未加 bot 等鉴权失败会落到 health/contract
// 的 drift 缺省文案（误导去改 src/llm/contract.ts），漏掉 #1 要直击的「登录/凭据失效」场景。
process.env.FORGE_DB = ':memory:';
import { test, mock } from 'node:test';
import assert from 'node:assert/strict';

// 仅替 port.probe()——probeFeishu 只用到它；避免真打飞书 API。
...
test('probeFeishu：未配齐 available=false → 透传、无 kind（不告警）', async () => {
...
test('probeFeishu：信封完好 ok=true → 无 kind', async () => {
